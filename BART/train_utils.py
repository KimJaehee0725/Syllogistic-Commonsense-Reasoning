...
def train(args, model, dataloader, tokenizer, fold_name = None, val_dataloader = None, scheduler = None) :
    ...
    for num, epoch in enumerate(range(args.num_epochs)) :
        for model_inputs in dataloader :
            ...
            if step % args.eval_interval == 0 and args.kfold != 0 :
...
def log_validation(args, model, val_dataloader, tokenizer, wandb, step, fold_name, is_final = False) :
    ...
    if (not is_final and args.save_generation):
        save_generation(f"{args.generation_path}/val_generation_{step}-steps_{fold_name}.csv", val_generation)
    
    if (is_final and args.save_final_model) :
        save_generation(f"{args.generation_path}/final_generation.csv", val_generation)

    print(f"valid bleu-1 : {val_bleu['bleu-1']}")
    print(f"valid bleu-2 : {val_bleu['bleu-2']}")
    print(f"valid bleu-3 : {val_bleu['bleu-3']}")
    print(f"valid rouge-1 : {val_rouge['rouge-1']}")
    print(f"valid rouge-2 : {val_rouge['rouge-2']}")
